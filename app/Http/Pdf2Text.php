...
abstract class PdfObjectBase           // extends  Object
{
    ...
    protected function ProcessEscapedCharacter($ch)
    {
        switch ($ch) {
            ...
            case 't':  $newchar = "\t";
                break;
            case 'b':  $newchar = chr(8);
                break;
            case 'v':  $newchar = chr(11);
                break;
            ...
            for ($i = 0, $count = count($matches['keyword']); $i < $count; $i++) {
                ...
                switch (strtolower($keyword)) {
                    case 'author':  $this->Author = $value;
                        break;
                    case 'creator':  $this->CreatorApplication = $value;
                        break;
                    case 'producer':  $this->ProducerApplication = $value;
                        break;
                    case 'title':  $this->Title = $value;
                        break;
                    case 'creationdate':  $this->CreationDate = $this->GetUTCDate($value);
                        break;
                    case 'moddate':  $this->ModificationDate = $this->GetUTCDate($value);
                        break;
                }
            }
...
class PdfTexterFont extends PdfObjectBase
{
    ...
    public function __construct($resource_id, $cmap_id, $font_type, $secondary_cmap_id = null, $pdf_objects = null)
    {
        ...
        switch ($this->FontType) {
            ...
            default:
                if (PdfToText::$DEBUG) {
                    warning("Unknown font type encountered for object #$resource_id, character map #$cmap_id.");
                }
        }
    }
...
class PdfTexterUnicodeMap extends PdfTexterCharacterMap
{
    ...
    // Min and max values of the character ranges
    private $RangeMax = -1;
...
class PdfTexterEncodingMap extends PdfTexterCharacterMap
{
    ...
    public function __construct($object_id, $definitions)
    {
        ...
        switch (strtolower($encoding_match['encoding'])) {
            case 'pdfdocencoding':  $this->Encoding = self::PDF_DOC_ENCODING;
                break;
            case 'macromanencoding':  $this->Encoding = self::PDF_MAC_ROMAN_ENCODING;
                break;
            case 'standardencoding':  $this->Encoding = self::PDF_STANDARD_ENCODING;
                break;
...
class PdfEncryptionData extends PdfObjectBase
{
    ...
    // User and owner password keys
    public $OwnerKey;

    public $UserEncryptionString;

    // Not sure yet of the real usage of these ones
    public $OwnerEncryptionString;
