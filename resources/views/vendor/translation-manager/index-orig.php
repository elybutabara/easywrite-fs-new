...
<div style="width: 80%; margin: auto;">
    ...
    <p>
        ...
        <?php endif; ?>
        <?php if(isset($group)) : ?>
            <form class="form-inline form-publish" method="POST" action="<?= action([\Barryvdh\TranslationManager\Controller::class, 'postPublish'], $group) ?>" data-remote="true" role="form" data-confirm="Are you sure you want to publish the translations group '<?= $group ?>? This will overwrite existing language files.">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
    ...
    <table class="table">
        ...
        <?php foreach($translations as $key => $translation): ?>
            <tr id="<?= $key ?>">
                ...
                <?php if($deleteEnabled): ?>
                    <td>
                        <a href="<?= action([\Barryvdh\TranslationManager\Controller::class, 'postDelete'], [$group, $key]) ?>" class="delete-key" data-confirm="Are you sure you want to delete the translations for '<?= $key ?>?"><span class="glyphicon glyphicon-trash"></span></a>
                    </td>
                <?php endif; ?>
            </tr>
        <?php endforeach; ?>
